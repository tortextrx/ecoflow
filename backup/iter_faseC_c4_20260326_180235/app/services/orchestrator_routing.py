...
def detect_active_flow(state: str, session: dict) -> Optional[str]:
    ...
    # Prioridad alta: campo pendiente explícito dentro de flujo ya abierto.
    if session.get("pending_field") and session.get("flow_mode"):
        return session.get("flow_mode")
...
def detect_proactive_history_intent(message: str, msg_c: str, intent: str, entities: dict) -> Optional[Dict[str, Any]]:
    ...
    # Nunca pisar consultas de campo de entidad (email, teléfono, dirección, etc.).
    if intent == "consultar_campo":
        return None
    if any(k in msg_c for k in ["email", "correo", "telefono", "teléfono", "direccion", "dirección"]):
        return None
    ...
    if found_pkey:
        ...
        is_history_keyword = any(k in msg_c for k in ["historia", "actuacio", "nota", "ver historial", "historial"])
...
def detect_new_flow(intent: str, msg_c: str) -> Optional[str]:
    ...
    if intent == "create_article":
        return "article"
    if has_create_signal and any(k in msg_c for k in ["articulo", "artículo", "producto", "referencia"]):
        return "article"
    if intent == "open_task" or any(k in msg_c for k in ["servicio", "tarea", "parte de trabajo", "parte"]):
        return "service"
    if intent == "create_contract":
        return "contract"
    if has_create_signal and "contrato" in msg_c:
        return "contract"
    ...
    # Evitar que búsquedas/listados de entidades entren por alta.
    entity_list_hints = ["dime los", "que hay", "listar", "lista", "clientes", "busca", "buscar", "muéstrame", "muestrame"]
    is_probable_listing = any(k in msg_c for k in entity_list_hints)
    if intent == "create_entity":
        if is_probable_listing and not has_create_signal:
            return None
        return "entity"
    if has_create_signal and any(k in msg_c for k in ["cliente", "proveedor", "entidad", "cif", "nif"]):
        return "entity"
    if is_probable_listing:
        return None
    return None
