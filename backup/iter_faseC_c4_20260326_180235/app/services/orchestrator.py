...
class UnifiedOrchestrator:
    ...
    async def _handle_query_contract_field(self, session: dict, message: str, entities: dict) -> dict:
        ...
        if not field:
            return {"reply": "¿Qué campo del contrato quieres consultar? (referencia, código, periodicidad, estado, bloque)", "state": "idle"}
        ...
        if target.get("status") != "RESOLVED":
            return {"reply": "No tengo un contrato activo. Indícame ID o un criterio de búsqueda.", "state": "idle"}

        contract = target.get("data", {})
        rb = await tool_registry.obtener_contrato.execute({"pkey": int(contract.get("pkey"))})
        if not rb.get("found"):
            return {"reply": f"No he podido leer el contrato {contract.get('pkey')}", "state": "idle"}
        raw = rb.get("data") or {}
        session["active_contract"] = {
        ...
        return {"reply": f"El campo **{field}** del contrato {contract.get('pkey')} es: {value}", "state": "idle"}
    ...
    async def _handle_list_contracts(self, session: dict, entities: dict, message: str | None = None) -> dict:
        ...
        if len(options) == 1:
            ...
            session["active_article"] = None
            session["active_service"] = None
            return {"reply": f"📄 Contrato {chosen['pkey']} seleccionado automáticamente (única coincidencia).", "state": "idle"}
    ...
    async def _handle_query_article(self, session: dict, entities: dict) -> dict:
        ...
        if not data:
            return {"reply": f"No encuentro artículos con '{hint_s}'.", "state": "idle"}
    ...
    async def _handle_query_article_field(self, session: dict, message: str, entities: dict) -> dict:
        ...
        if not field:
            return {"reply": "¿Qué dato del artículo necesitas? (referencia, proveedor, familia, marca, estado)", "state": "idle"}
        ...
        if not article:
            return {"reply": "No tengo un artículo activo. Indícame nombre o referencia primero.", "state": "idle"}

        rb = await tool_registry.obtener_articulo.execute(int(article.get("pkey")))
        if not rb.get("found"):
            return {"reply": f"No he podido leer el artículo {article.get('pkey')}", "state": "idle"}
        ...
        return {"reply": f"El campo **{field}** del artículo {article.get('pkey')} es: {value}", "state": "idle"}
    ...
    async def _handle_disambiguation(self, session: dict, message: str) -> dict:
        ...
        if pa and pa.get("intent") in {"list_articles", "modify_article_select", "query_article_field_select"}:
            ...
            return {"reply": f"Seleccionado artículo: **{article['descripcion']}** (ID {article['pkey']}).", "state": "idle"}
        ...
        if pa and pa.get("intent") in {"list_contracts", "modify_contract_select", "query_contract_field_select"}:
            ...
            return {"reply": f"Seleccionado contrato **{contract['pkey']}**. ¿Qué dato necesitas?", "state": "idle"}
    ...
    async def _handle_last_service_followup(self, session: dict, msg_c: str) -> dict:
        ...
        if not sid:
            return {"reply": "No tengo un servicio reciente activo en esta conversación.", "state": "idle"}
        ...
        if any(k in msg_c for k in ["comprueba", "comprobar", "verifica", "verificar"]):
            ...
            if r.get("success"):
                ...
                return {"reply": f"✅ Servicio {sid} identificado y verificado por lectura.", "state": "idle"}
            return {"reply": f"❌ No he podido verificar por lectura el servicio {sid}: {r.get('error') or 'error de tool/API'}.", "state": "idle"}

        if any(k in msg_c for k in ["numero", "número", "id", "pkey", "se ha creado"]):
            return {"reply": f"El servicio creado es el **{sid}**.", "state": "idle"}

        if any(k in msg_c for k in ["fecha", "quedo", "quedó"]):
            fecha = (svc.get("fecha_hora_text") or "").strip()
            if fecha:
                return {"reply": f"Quedó para: **{fecha}**.", "state": "idle"}
            r = await tool_registry.obtener_servicio.execute({"pkey": int(sid)})
            if r.get("success"):
                data = r.get("data") or {}
                fecha_api = data.get("FECHA_INICIO") or data.get("FECHA") or "(sin fecha)"
                return {"reply": f"La fecha del servicio {sid} es: **{fecha_api}**.", "state": "idle"}
            return {"reply": f"No he podido recuperar la fecha del servicio {sid}: {r.get('error') or 'error de lectura'}.", "state": "idle"}

        return {"reply": f"Servicio activo: {sid}. Puedo darte número, fecha o verificar creación.", "state": "idle"}
