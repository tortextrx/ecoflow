...
except Exception:  # pragma: no cover - fallback explícito para entornos sin dependencia
    fuzz = None

from app.services.tools.registry import tool_registry
from app.services.normalizers import normalize_text, normalize_cif_nif
...
class ResolverService:
    ...
    def _normalize_string(self, text: str) -> str:
        if not text:
            return ""
        return re.sub(r"[^\w\s]", "", normalize_text(text))

    def _normalize_cif(self, cif: str) -> str:
        return normalize_cif_nif(cif)
    ...
    def _name_similarity_ratio(self, target: str, candidate: str) -> float:
        """Motor principal RapidFuzz; fallback determinista a SequenceMatcher."""
        if fuzz is not None:
            # fuzz.ratio devuelve 0..100
            return float(fuzz.ratio(target, candidate)) / 100.0
    ...
    def _rank_similar_names(self, name: str, candidates: List[dict], threshold: float = 0.84) -> List[dict]:
        ...
        for it in candidates:
            ...
            if not cand_name:
                continue
            score = self._name_similarity_ratio(target, cand_name)
            if score >= threshold:
